<script>
    ...
    function edit(id, title, description, image) {
        if(description=='_____') description="";
    ...
    function display_achievement_list() {
        $.ajax({
            type: "POST",
            url: "<?= base_url('administrator/display_achievement_list') ?>",
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    $.each(data, function(i, data) {
                        ...
                        if(data.description=="") data.description='_____';

                        str += `<tr>
                        <td>${i+1}</td>
                        <td><img src="<?= apiBaseUrl ?>${data.image_path}" width="50px"></td>
                        <td class="title"><a href="<?= base_url("administrator/data_details/") ?>${data.uid}">${data.title}</a></td>
                        <td class="title">${data.description}</td>
                        <td>
                        <div>

                        <button class="hdrbtn mx-2 edit_user access_update" data-toggle="modal" id=" editbtn"  data-target="#edtView1"  onclick="edit('${data.uid}' , '${data.title}', '${data.description}', '${data.image_path}')" data-toggle="tooltip" data-placement="top" title="Edit" disabled>
    ...
    $('#table_details').on('click', '.delete_user', function() {
        ...
        $('#btn_delete_data').click(function() {
            $.ajax({
                type: "post",
                url: "<?= base_url('administrator/delete_achivements') ?>",
                data: {
                    'id': id
                },
                async: false,
    ...
    $('#add_data_form').submit(function(e){
        ...
        $.ajax({
            type:"POST",
            url:"<?= apiBaseUrl?>common/addAchivements",
            headers:{
    ...
    $('#update_form').submit(function(e){
        ...
        $.ajax({
            type:"POST",
            url:"<?= apiBaseUrl?>common/editAchivements",
            headers:{
                'x-api-key':'<?=const_x_api_key?>',
                'platform':'web',
                'deviceid':''
