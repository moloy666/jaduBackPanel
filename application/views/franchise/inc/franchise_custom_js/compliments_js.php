<script>
    ...
    function display_compliments_list() {
        $.ajax({
            type: "POST",
            url: "<?= base_url('administrator/display_compliments_list') ?>",
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    $.each(data, function(i, data) {
                        ...
                        str += `<tr>
                        <td>${i+1}</td>
                        <td><img src="<?= apiBaseUrl ?>${data.image}" width="50px"></td>
                        <td class="title">${data.name}</td>
                        <td>
                        <div>


                        <button class="hdrbtn mx-2 edit_user access_update" data-toggle="modal" id=" editbtn"  data-target="#edtView1"  onclick="edit('${data.uid}' , '${data.name}', '${data.image}')" data-toggle="tooltip" data-placement="top" title="Edit" disabled>
    ...
    $('#table_details').on('click', '.delete_user', function() {
        ...
        $('#btn_delete_data').click(function() {
            $.ajax({
                type: "post",
                url: "<?= base_url('administrator/delete_compliments') ?>",
                data: {
                    'id': id
                },
                async: false,
    ...
    $('#add_data_form').submit(function(e){
        ...
        let specific_id = $('#specific_id');

        formData.append('specific_id', specific_id);
        
        $.ajax({
            type:"POST",
            url:"<?= apiBaseUrl?>common/addCompliments",
            headers:{
    ...
    $('#update_form').submit(function(e){
        ...
        $.ajax({
            type:"POST",
            url:"<?= apiBaseUrl?>common/editCompliments",
            headers:{
                'x-api-key':'<?=const_x_api_key?>',
                'platform':'web',
                'deviceid':''
