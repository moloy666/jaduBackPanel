...
<body>
                            ...
                            foreach ($customer as $i => $value) {
                                if (!empty($value)) { ?>
                                    <tr>
                                        ...
                                        <td class="text-center"><?= $value[0]['name'] ?></td>
                                        <td class="text-center"><?= $value[0]['mobile'] ?></td>
                                        <td class="text-center"><?= $value[0]['email'] ?></td>
                                        <td class="text-center">
                                            <label class="switch">
                                                <input type="checkbox" <?= ($value[0]['status'] == const_active) ? "checked" : "" ?> id="check-status" data="<?= $value[0]['user_id'] ?>" onclick="status(this, '<?= $value[0]['user_id'] ?>')">
    ...
    <script>
        ...
        function get_driver_details() {
            let sarathi_id = $('#specific_id').val();
            $.ajax({
                type: "post",
                url: "<?= base_url(WEB_PORTAL_SARATHI . '/get_driver_data') ?>",
                data: {
                    "id": sarathi_id
                },
        ...
        function get_customer_data(driver_id) {
            $.ajax({
                type: "POST",
                url: "<?= base_url(WEB_PORTAL_SARATHI . '/get_customer_data') ?>",
                data: {
                    "id": driver_id
                },
                errorZ: function(response) {
                    console.log(response);
                },
                success: function(response) {
                    // console.log(response);
                    let data = response.data;
                    $.each(data, function(i, data) {
                        get_customer_details(data.customer_id);
                    });
                }
            });
        }
        ...
        function get_customer_details(customer_id) {
            $.ajax({
                type: "POST",
                url: "<?= base_url(WEB_PORTAL_SARATHI . '/get_customer_details') ?>",
                data: {
                    "id": customer_id
                },
                ...
                success: function(response) {
                    ...
                    $.each(data, function(i, data) {
                        ...
                        <input type="checkbox"  ${user_status} id = "check-status" data ="${data.user_id}" onclick="status(this, '${data.user_id}')">
                        <span class="slider round"></span></label>
                        </td>
                        <td>
                                <div>
    ...
    <!-- <script>
        $.ajax({
            type:"POST",
            url:"<?= base_url('admin/get_driver_ids') ?>",
            data:{
                "sarathi_id":$('#specific_id').val()
            },
            error:function(response){
                console.log(response);
            },
